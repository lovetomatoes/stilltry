...
\tablehead{
  ...
  \colhead{Redshift} && \colhead{$\Phi^\ast$} & \colhead{$\Muv^\ast$} & \colhead{$\tilde \alpha$} & \colhead{$ \tilde \beta$}& &
  \colhead{$\Phi^\ast$} & \colhead{$\Muv^\ast$} & \colhead{$\tilde \alpha$} & \colhead{$\tilde \beta$}\\
...
\enddata
\tablecomments{Parameters characterizing the double power-law fitting functions
of both the unobscured and intrinsic QLFs.
At $z=6-10$, the QLF evolution is produced by the best-fit BH growth model with $\fseed=0.01$.
}
\end{deluxetable*}
...
\enddata
\tablecomments{\blue{
Predicted number of quasars by the BH growth model with $\fseed=0.01$ at each redshift in the two surveys, with the assumption of the survey volume with $\Delta z =1$ and 100\% completeness.
The integration of QLF starts from the 5$\sigma$ point source detection limits of the $YJH$ bands in each survey (27 mag for the RST; 24 mag for Euclid), 
and from the bright absolute magnitude with $\Muv<=-26$ mag.
In addition, with more realistic quasar identification capability down to 23 mag in the $YJH$ bands,
\citet{2019Barnett} predicts the yields of quasar numbers as 117 at $7<z<7.5$, 25 at $7.5<z<8.5$, and 2 at $8.5<z<9$.
We set the same detection limit and find our result in good agreement with theirs.}
}
\end{deluxetable*}
