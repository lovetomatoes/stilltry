...
it still remains challenging to develop a theoretical model that explains its redshift evolution associated with BH growth self-consistently.
...
The predicted redshift evolution of the QLF and BHMF of the earliest BHs suggests a rapid decay of their number and mass density 
...
For the two cases, the best-fit solution consistently yields a small value of $\delta < 0.1$.
...
Thus, a more significant fraction of BHs actively grow with $\lambda \gtrsim 1$ and contribute to developing of the QLF and BHMF.
...
Since our model is calibrated against the $z=6$ BHMF of \citetalias{2010AJ....140..546W},
...
The mass range we use for fitting is $10^6~\Msun \leq M_\bullet \leq 10^{10}~\Msun$.
}
\end{deluxetable}
...
The mass density of BHs formed in the biased, overdense region of the universe %(haboring DM halos with $\Mh\gtrsim 10^{11}~\Msun$) 
...
The number density of quasars is shown from $z=6$ to $10$ with a step of 1.
\blue{Selection completeness is assumed to be 100\% below the magnitude limits considered.
The integration of the QLF is conducted for quasar populations brighter than the photometric depth $m_{1450}$,
inferred from the 5$\sigma$ point source detection limits of the $YJH$ bands in each survey.
To mimic realistic quasar identification capability, we show the detection numbers with $m_{1450}\leq 23$ mag in comparison with}
...
\blue{We show the prediction of no detection or no prediction in the literature with the symbol ``--''.}
}
\end{deluxetable*}
