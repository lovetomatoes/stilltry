...
the parameterized BH growth and how we reproduce the modeled QLF and BHMF dataset at $z\sim$ 6 using these ingredients. 
...
% wli avoid fseed=1
We take three different values of $\fseed = 1$, $0.1$, and $0.01$ and explore the dependence on the best-fit QLF and BHMF.
...
For both cases, the best-fit solution consistently shows a small value of $\delta < 0.01$. \red{KI: but, $\delta=0.055$ is used later...}
%, which is sufficiently small and should be marginalized out. 
This suggests that a nearly exponential BH growth manner is favored in our model assumption. 
...
\begin{figure*}
\centering
\includegraphics[width=160mm]{BHMF_rhoz1.png}
\caption{
\if0
...
We apply the result to construct BHMFs and QLFs at higher redshifts and give implications for
...
with both cases of $\fseed=0.1$ and 0.01.
The seed BH mass function is already established with $M_\bullet \lesssim 2\times10^5~\Msun$ earlier than $z\sim 17$,
so the BHMF at mass ranges higher than the typical seed mass is grown from a portion of $\fseed$ of all seeds.
Therefore, with $\fseed=0.1$, the number of growing seeds is 10 times higher than in the $\fseed =0.01$ case,
which is partially reflected by the difference of BHMFs at $M_\bullet \sim 10^{5-6}~\Msun$ in their early growth.
As for $M_\bullet \gtrsim 10^7~\Msun$, which is potentially accessible by quasar observations,
in all redshift bins, the BHMFs can be characterized with a double power-law function,
...
Constrained by the \citetalias{2010AJ....140..546W} BHMF with $M_\bullet \sim 10^8-3\times 10^9~\Msun$ at $z=6$, 
the two $\fseed$ cases produce similar BHMF at in this mass and redshift range (see \S~\ref{sec:fitting_result}),
while in the low mass range ($M_\bullet \lesssim 10^7~\Msun$), the difference in the two BHMF shapes is profound.
The difference is also shown by the mass ranges where the slope of BHMF is close to $\alpha_{M_\bullet}\simeq -1$,
i.e., the typical masses which dominate the cumulative mass density
(see \S~\ref{sec:fitting_result}). In the case of $\fseed=0.01$, this typical mass range is higher,
despite the integrated mass density is lower than that with $\fseed=0.1$. 


In the right panel of Fig.~\ref{fig:BHMF_rhoz}, 
we also present the evolution of BH cumulative mass density in a comoving volume,
defined by the integration of the BHMF as
%
\begin{equation}
 \rho_\bullet(z)=\int_{\mathcal{D}}^{} \Phi_{\Mbh} (z) \Mbh \D \log \Mbh,
\end{equation}
%
where the mass ranges are $\mathcal{D} = M_{\bullet}<10^6$,
$10^6<M_{\bullet}<10^8$, and $M_\bullet>10^8~\Msun$.
% Varying the mass ranges as $\mathcal{D} = [,10^5]$, $[10^5,10^6]$, $[10^6,10^8]$, and $[10^8,]~\Msun$,
% Varying the mass ranges as $\mathcal{D} = \{M_{\bullet}<10^5\}, \{M_{\bullet} \rm{in} 10^{5-6}\}, 
% 10^{6-8}$, and $M_{\bullet}>10^8~\Msun$,
...
For comparison, we overlay the results of BH mass density evolution with $10^6<M_{\bullet}<10^8$, and $M_\bullet>10^8~\Msun$
from cosmological simulations in \citet{2022MNRAS.513..670N}, with the dashed-dotted line and square colored according to the mass ranges.
At lower redshifts $z\lesssim 7$, our results are in overall good agreement with theirs.
However, our models predict more $10^{6-8}~\Msun$ BHs emerging at $z\sim 10-8$,
owing to the joint effect of our seeding scenario and growth model,
...
The upper limit from the integrated X-ray emission \citep{2013ApJ...778..130T} is denoted by red triangles.
The total BH mass density should be dominated by abundant low mass BHs, which is not comprised in our quasar host halos.
In this sense, our BHs which result in observable quasars at $z\sim 6$ are a biased population.
...
\begin{figure}
\centering
\includegraphics[width=85mm]{M0r8_f2QLF.png} % width=160 QLF_rhoz.png
\caption{
...
\subsection{Quasar number density evolution}
\label{sec:qlfglf}
\blue{(my revision on Fig.~\ref{fig:QLF_rhoz} description)
Next, in Fig.~\ref{fig:QLF_rhoz} we show unobscured quasar number densities at different magnitudes evolving with redshift.
Based on our best-fit parameters in the BH growth model, 
we predict the number densities evolution at $z\sim 6-10$, of quasars at $\Muv=-23$ to $-27$ mag.
We also show the quasar number densities from previous observational studies, depicting the cosmic QLF evolution from $z\sim 0-6$
\citep{2006AJ....131.2766R,2008ApJ...675...49S,2013ApJ...768..105M,2018PASJ...70S..34A,2018ApJ...869..150M,2020ApJ...904...89N}.
Peaking at $z\sim 2-3$, the quasar number densities at all magnitudes decline exponentially toward high redshifts.
A rapid decay of the quasar number density toward higher redshifts is found by previous studies
...
From our theoretical prediction, the evolutionary trend of luminous quasar number density of with $\Muv <-26~{\rm mag}$ can
be fitted as $\propto 10^{k_{\rm L}z}$, where $k_{\rm L}\approx -0.97$.
