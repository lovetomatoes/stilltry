...
% \begin{equation*}
%   \label{eq:mdot_1}
%   \Mdot_\bullet = \frac{\lambda f(\Mbh) \Mbh}{\tEdd} ,
% \end{equation*}
\begin{equation}
  \label{eq:mdot}
...
$\tEdd =  \Mbh/\Mdot_{\mathrm{Edd}} \approx 45$ Myr.
...
With those values, the number fraction of BHs growing with Eddington ratios above unity is
estimated as $P(>1)=***$.
...
Constrained by $z\sim 6$ observations, 
the best-fit model from the MCMC realizations enables us to probe the statistical BH evolution at higher redshifts.
...
We apply the result to construct BHMF and QLF at higher redshifts 
and produce implications to high-$z$ observations in the future.

The best-fit modeled BHMFs with 1-$\sigma$ scatter at redshifts $z$ from 6 to 10 is shown in  Fig.~\ref{fig:BHMF_rhoz}.
...
The mass density of BHs with $\Mbh \gtrsim 10^7~\Msun$ is integrated by the BHMF as
\begin{equation}
  \rho_\bullet(z)=\int_{10^7~\Msun}^{\infty} \Phi_{\Mbh} (z) \Mbh \D \log \Mbh, 
\end{equation}
as a function of redshift.
Thus we predict the early assembly of BH mass in the galactic nuclei, 
and the steep decline toward higher redshifts can be described by a fitting formula $\rho_\bullet(z)\propto 10^{k_{\rm M}(z-6)}$,
with $k_{\rm M}\approx -0.8$. 
The exponential dependency on redshift is reminiscent of the cosmic galaxy mass growth 
...
On the other hand, the QLFs at $z$ from 6 to 10 are also deduced from the BH growth model (see Fig.~\ref{fig:QLF_rhoz}).
...
The luminous quasar ($\Muv <-26$) density evolution with redshift is shown in Fig.~\ref{fig:QLF_rhoz}, and fitted with 
$n(z,\Muv<-26)\propto 10^{k_{\rm L}z}$, where $k_{\rm L}\approx -0.95$. 
A rapid decline in luminous quasar population toward high redshift is firstly proposed by \citet{2001AJ....122.2833F}, 
with $k_{\rm L}= -0.47$ in their analysis of $z$ from 3 to 6 quasar sample. 
A steeper decline in higher redshifts is suggested in recent studies of $z\gtrsim 6$ quasars, 
...
In this work we argue that the episodical BH accretion with bursts and quiescent phases is 
